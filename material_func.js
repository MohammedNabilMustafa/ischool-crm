...
async function MATERIAL_DASHBOARD()
{
    Loading_page_set();
    ...
    document.getElementById("Location_1").innerHTML +=`<label> Recorded URL : </label><input class='col-9' id='recorded_url_input' type='text'/>`;

    document.getElementById("Location_1").innerHTML +=`</div></div>`;

    document.getElementById("Location_1").innerHTML +=`<div class="col"><div class="form-floating mb-3 search_adjust">`;

    document.getElementById("Location_1").innerHTML +=`<label> Slide URL : </label><input class='col-9' id='slide_url_input' type='text'/>`;
  ...
  document.getElementById("Location_1").innerHTML +=`</div></div>`;
    ...
    create_table_material_func(get_material_arr);
    ...
    $('#send_group').click(async function () {
        ...
        input_send_data[input_send_data_count] = $("#slide_url_input").val();input_send_data_count++;
        ...
        if(input_send_data[4] == ""){alert("Set Recorded URL");Loading_page_clear();return;}
        if(input_send_data[6] == ""){alert("Set Session Number");Loading_page_clear();return;}

        var response = await ADD_DATA_TABLES_ONE_COL(database_fixed_link , 'material' ,
        [
            "level_id",
            "lan_id",
            "session_type_id",
    ...
    $('#search_btn').click(async function (index) {  
        ...
        var get_marketing_arr_search = await GET_DATA_TABLES(database_fixed_link , 'material' );
        create_table_material_func(get_marketing_arr_search);
 ...
 async function create_table_material_func(data_table)
 {
    var get_lvl_arr = await GET_DATA_TABLES(database_fixed_link , 'level' );
    var get_lan_arr = await GET_DATA_TABLES(database_fixed_link , 'lan' );
    var get_session_type_arr = await GET_DATA_TABLES(database_fixed_link , 'session_type' );
    ...
    if(values_ == "")
    {
       ...
       createTable_material( data_table ,All_data_obj ); ;
       return;
    }
    var result = [];
    if(data_table)
    {
     ...
     createTable_material(result ,All_data_obj , 'clear'); 

}

function createTable_material(dataArray ,All_data_obj ) {
    ...
    if(dataArray && dataArray !== undefined && dataArray.length != 0){
      ...
      var result = "<table class='table' id='dtable'>"+
               ...
               for(var index = 0 ; index < Object.values(dataArray).length; index ++)
               {
                ...
                result += `<a href="${dataArray[index].video_link}" target="_blank">Session Recorded ${dataArray[index].session_num} link</a> <br>`;
                if(dataArray[index].slide != "No Slide")
                {
                    result += `<a href="${dataArray[index].slide}" target="_blank">Session Slide ${dataArray[index].session_num} link</a>`;
                }
                else
                {
                    result += `${dataArray[index].slide}`;
                }
                result += "</td>";
                result += "</tr>";
               }
