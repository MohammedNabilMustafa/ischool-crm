...
async function ADD_sales_call()
{
    ...
    add_select_html_students_sales_search("Location_1" , "search_s" , "Search ST/PT " , students_arr_0 );
...
<button type="button" id='exp' class="btn btn-info" hidden>
Expired Customer
</button>
<button type="button" id='ar' class="btn btn-info">
Registered Customer
</button>
<button type="button" id='paused' class="btn btn-info" hidden>
PAUSED Customer
</button>
  </div> `;
  ...
  document.getElementById("Location_1").innerHTML +=`</div></div>`;
    ...
    document.getElementById("Location_1").innerHTML +=`</div></div>`;
    document.getElementById("Location_1").innerHTML +=`<div class="col"><div class="form-floating mb-3 search_adjust">`;
    add_select_html_invoice_sales("Location_1" , "call_invoice_input" , "Invoices " );
...
<button type="button" id='follow_up_btn_ar' class="btn btn-danger" hidden>
AR Follow Up
</button>

<button type="button" id='follow_up_btn_prob' class="btn btn-danger" hidden>
Problem Follow Up
</button>

<button type="button" id='follow_up_btn_new' class="btn btn-danger" hidden>
New Follow Up
</button>
...
<button type="button" id='follow_up_btn_date_ar' class="btn btn-danger" hidden>
AR Follow Up
</button>
<button type="button" id='follow_up_btn_date_prob' class="btn btn-danger" hidden>
Problem Follow Up
</button>
<button type="button" id='follow_up_btn_date_new' class="btn btn-danger" hidden>
New Follow Up
</button>
</div> `
...
document.getElementById("Location_1").innerHTML +=`</div></div>`;
  ...
  check_sales_body_once_sales(parent_arr_select);
  check_sales('#all_cust' , parent_arr_select);
  check_sales('#follow_up' , parent_arr_select);
  check_sales('#ar' , parent_arr_select);
  check_sales('#not_called' , parent_arr_select);
  check_sales('#exp' , parent_arr_select);
  check_sales('#paused' , parent_arr_select);
...
function add_select_html_students_sales (location_, paper_ , title , data_arr)
{
  ...
  if(data_arr && data_arr !== undefined && data_arr.length != 0){
    for(var index = 0 ; index < Object.values(data_arr).length ; index++)
    {
        result +=  `<option value="${data_arr[index].id} "> ( ID : ${data_arr[index].id} ) - ( Name : ${data_arr[index].st_name} )</option>` 
    }
}
  result+=`</select>` ;
document.getElementById(location_).innerHTML += result
...
function add_select_html_invoice_sales (location_, paper_ , title , data_arr)
{
  result = `<label for="`+paper_+`">`+title+`:</label>
  <select class="col-9 select2" name="`+paper_+`" id="`+paper_+`">
...
function add_select_html_students_sales_search (location_, paper_ , title , data_arr)
{
  result = `<label for="`+paper_+`">`+title+`:</label>
  <select class="col-9 select2" name="`+paper_+`" id="`+paper_+`">
  ...
  if(data_arr && data_arr !== undefined && data_arr.length != 0){
    for(var index = 0 ; index < Object.values(data_arr).length ; index++)
    {
        result +=  `<option value="${data_arr[index].id} "> (PT-ID : ${data_arr[index].id}) - (PT-Name : ${data_arr[index].name} ) - (ST-ID : ${data_arr[index].stid}) - (ST-Name : ${data_arr[index].st_name})</option>` 
    }
}
  result+=`</select>` ;
document.getElementById(location_).innerHTML += result
...
function check_sales(id_btn , arr_data){
    ...
    $(id_btn).click(async function () { 
        ...
        var parent_arr_select = await check_sales_body(id_btn , arr_data);

    });

}

async function check_sales_body(id_btn , arr_data_all)
{
    var arr_data = [];
    var arr_data_count = 0;

    var arr_data_not = [];
    var arr_data_not_count = 0;
    ...
    arr_data_all.forEach(element => {
        ...
        if(check_parent)
        {
            arr_data_not[arr_data_not_count] = element;arr_data_not_count++;
    ...
    else if (id_btn == '#ar')
    {
        ...
        parent_arr_select = arr_data_not;
        student_arr_select = students_arr_0;
        invoice_arr_select = customer_inv;

        arr_data_not.forEach(element => {
            $('#call_parent_input').append(`<option value="${element.id} "> ( ID : ${element.id} ) - ( Name : ${element.name} )</option>` );   

        });

        $(id_btn).text(`Registered Customer (${parent_arr_select.length}) `);
...
async function check_sales_body_once_sales(arr_data_all)
{
    var arr_data = [];
    var arr_data_count = 0;
    ...
    arr_data_all.forEach(element => {
        ...
        parent_arr_select = arr_data_not;
        $('#ar').text(`Registered Customer (${parent_arr_select.length}) `);
